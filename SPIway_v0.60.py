...
# Copyright (C) 2020 mh.nasr@live.com
#
...
#		print "Raw ID data: 0x%02x 0x%02x"%(ord(spi_info[0]), ord(spi_info[1]))
		...
		self.DEVICE_ID_0 = ord(spi_info[1])
		self.DEVICE_ID_1 = ord(spi_info[2])
	...
	def printstate(self):
		...
		if self.MF_ID == 0xC2:
			...
			if self.DEVICE_ID_0 == 0x20 and self.DEVICE_ID_1 == 0x19:
				print "Chip type:         MX25L25635F (0x%02x)"%self.DEVICE_ID_0
			...
			elif self.DEVICE_ID_0 == 0x20 and self.DEVICE_ID_1 == 0x11:
				print "Chip type:         MX25L1006E (0x%02x)"%self.DEVICE_ID_0
				...
				print "Chip type:         unknown (0x%02x)"%self.DEVICE_ID_0
				self.close()
				sys.exit(1)

		elif self.MF_ID == 0xEF:
			print "Chip manufacturer: Winbond (0x%02x)"%self.MF_ID
			if self.DEVICE_ID_0 == 0x10:
				print "Chip type:         W25X10CL (0x%02x)"%self.DEVICE_ID_0
				self.SPI_BLOCK_COUNT = 2
				self.SPI_SECTORS_PER_BLOCK = 16
				self.SPI_SECTOR_SIZE = 0x1000
				self.SPI_TOTAL_SECTORS = self.SPI_SECTORS_PER_BLOCK * self.SPI_BLOCK_COUNT
				...
				self.SPI_ADDRESS_LENGTH = 3
				self.SPI_USE_3BYTE_CMDS = 0
			elif self.DEVICE_ID_0 == 0x13:
				print "Chip type:         W25Q80BV (0x%02x)"%self.DEVICE_ID_0
				...
				self.SPI_ADDRESS_LENGTH = 3
				self.SPI_USE_3BYTE_CMDS = 0
			elif self.DEVICE_ID_0 == 0x40 and self.DEVICE_ID_1 == 0x19:
				print "Chip type:         W25Q256FV (0x%02x)"%self.DEVICE_ID_0
				self.SPI_BLOCK_COUNT = 512
				self.SPI_SECTORS_PER_BLOCK = 16
				self.SPI_SECTOR_SIZE = 0x1000
				self.SPI_TOTAL_SECTORS = self.SPI_SECTORS_PER_BLOCK * self.SPI_BLOCK_COUNT
			...
			elif self.DEVICE_ID_0 == 0x70 and self.DEVICE_ID_1 == 0x15:
				print "Chip type:         25Q16JVXXM (0x%02x)"%self.DEVICE_ID_0
				...
				self.SPI_ADDRESS_LENGTH = 3
				self.SPI_USE_3BYTE_CMDS = 0
			elif self.DEVICE_ID_0 == 0x40 and self.DEVICE_ID_1 == 0x15:
				print "Chip type:         25Q16JVXXQ (0x%02x)"%self.DEVICE_ID_0
				self.SPI_BLOCK_COUNT = 32
				self.SPI_SECTORS_PER_BLOCK = 16
				self.SPI_SECTOR_SIZE = 0x1000
				self.SPI_TOTAL_SECTORS = self.SPI_SECTORS_PER_BLOCK * self.SPI_BLOCK_COUNT
			...
			print "Chip manufacturer: Cypress/Spansion (0x%02x)"%self.MF_ID
			if self.DEVICE_ID_0 == 0x60 and self.DEVICE_ID_1 == 0x19:
				print "Chip type:         S25FL256L (0x%02x)"%self.DEVICE_ID_0
				self.SPI_BLOCK_COUNT = 512
				self.SPI_SECTORS_PER_BLOCK = 16
				self.SPI_SECTOR_SIZE = 0x1000
				self.SPI_TOTAL_SECTORS = self.SPI_SECTORS_PER_BLOCK * self.SPI_BLOCK_COUNT
			...
			print "Chip type:         unknown (0x%02x)"%self.DEVICE_ID_0
			print "Chip model:        unknown (0x%02x)"%self.DEVICE_ID_1
			self.close()
			sys.exit(1)
...
if __name__ == "__main__":
	...
	print "Copyright (C) 2017 judges@eEcho.com (Judges)"
	print "Copyright (C) 2020 mh.nasr@live.com (Abkarino)"
	print
