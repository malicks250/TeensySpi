...
class SPIFlasher(TeensySerial):
    ...
    #         self.write(self.CMD_PULLUPS_DISABLE)
        ...
        spi_info = self.read(4)

        # print "Raw ID data: 0x%02x 0x%02x 0x%02x 0x%02x" % (ord(spi_info[0]), ord(spi_info[1]), ord(spi_info[2]), ord(spi_info[3]))

        self.MF_ID = ord(spi_info[0])
        self.DEVICE_ID = ord(spi_info[1])
        self.MEM_TYPE = ord(spi_info[2])
        self.CAPACITY = ord(spi_info[3])
    ...
    # Clears the non-volatile block-protection bits (BP0-BP3) in the status register(s)
    # and releases any individually-locked blocks. Some PS4 motherboards (e.g. those
    # with a CXD90042 Southbridge) ship with these bits set or hold WP# asserted, which
    # makes every erase/write fail verification starting at block 0 even though reads
    # work fine. Retries a few times since the first status-register write can be
    # rejected by the chip if WP# was still asserted at the time it was issued.
    def unlockProtection(self, maxAttempts = 3):
        for attempt in range(1, maxAttempts + 1):
            self.write(self.CMD_SPI_UNLOCK)
            responseCode = self.readbyte()

            if responseCode == self.REQ_SUCCESS:
                return

            if responseCode == self.REQ_WRITE_PROTECTED and attempt < maxAttempts:
                print "Chip is still write-protected, retrying unlock (attempt %d/%d)..." % (attempt, maxAttempts)
                continue

            self.close()
            if responseCode == self.REQ_WRITE_PROTECTED:
                raise SPIError("Unable to clear write protection after %d attempt(s). The chip's WP# pin may be "
                                "hardware-tied on this motherboard and require a manual jumper/patch wire before "
                                "writing will succeed." % maxAttempts)
            elif responseCode == self.REQ_CMD_NOT_RECOGNIZED:
                command = self.readbyte()
                raise SPIError("Teensy firmware does not recognize the unlock command (0x%02x). Please reflash the "
                                "Teensy with the updated TeensySPIFlasher.ino that includes CMD_SPI_UNLOCK." % command)
            else:
                raise SPIError("Unlock failed with unexpected response code: %d" % responseCode)
    ...
    # skipErase: set when the caller already erased the whole chip up front (see program())
    def programBlock(self, data, block, skipErase = False):
        datasize = len(data)
        if datasize != self.SPI_BLOCK_SIZE:
            print "Incorrect length %d != %d!" % (datasize, self.SPI_BLOCK_SIZE)
            return -1

        # Erase the block before writing to it, unless it was already erased as part of a full-chip erase
        if not skipErase:
    ...
    def getSpiInfo(self):
        ...
        if self.MF_ID == 0xC2:
            ...
            if self.DEVICE_ID == 0x18:
                print "Chip type:         W25Q256 (0x%02x)" % self.DEVICE_ID
                self.SPI_BLOCK_COUNT = 512
                self.SPI_SECTORS_PER_BLOCK = 16
                self.SPI_SECTOR_SIZE = 0x1000
                self.SPI_TOTAL_SECTORS = self.SPI_SECTORS_PER_BLOCK * self.SPI_BLOCK_COUNT
            ...
            # Accept any memory-type byte here since it varies across Spansion/Cypress sub-families
            # (S25FL-K/S/L etc. all report different values), but still require the capacity byte to
            # confirm this is actually a 32MB part. Manufacturer ID 0x01 alone is not size-specific -
            # AMD/Spansion/Cypress also made 8/16MB parts under the same ID, and assuming 32MB for
            # those would make dump/vwrite address blocks that don't exist on the real chip.
            if self.CAPACITY == 0x19:
                print "Chip type:         FL256L / S25FL256L (type 0x%02x, capacity 0x%02x)" % (self.MEM_TYPE, self.CAPACITY)
                self.SPI_BLOCK_COUNT = 512
                self.SPI_SECTORS_PER_BLOCK = 16
                self.SPI_SECTOR_SIZE = 0x1000
                self.SPI_TOTAL_SECTORS = self.SPI_SECTORS_PER_BLOCK * self.SPI_BLOCK_COUNT
                ...
                print "Chip type:         Unknown (type 0x%02x, capacity 0x%02x)" % (self.MEM_TYPE, self.CAPACITY)
                print "Refusing to guess: capacity byte doesn't match the known 32MB FL256L/S25FL256L."
                self.close()
                sys.exit(1)
    ...
    def program(self, data, blockOffset, nblocks):
        ...
        print "Clearing flash write-protection (status register + block locks)..."
        self.unlockProtection()

        # For a full-chip write, one chip erase is dramatically faster than erasing all 512
        # blocks individually (each block erase pays its own erase-cycle time). Partial/ranged
        # writes still erase block-by-block so blocks outside the range are left untouched.
        fullChipWrite = (blockOffset == 0) and (nblocks == self.SPI_BLOCK_COUNT)
        if fullChipWrite:
            print "Erasing entire chip (faster than erasing each block individually)..."
            self.eraseChip()

        print "Writing %d blocks to device (starting at offset %d)..." % (nblocks, blockOffset)

        for block in range(blockOffset, (blockOffset + nblocks), 1):
            if not self.programBlock(data[block * self.SPI_BLOCK_SIZE:(block + 1) * self.SPI_BLOCK_SIZE], block, fullChipWrite):
                break
            print "\r%d KB / %d KB" % ((block - blockOffset + 1) * self.SPI_BLOCK_SIZE / 1024, nblocks * self.SPI_BLOCK_SIZE / 1024),
            sys.stdout.flush()
